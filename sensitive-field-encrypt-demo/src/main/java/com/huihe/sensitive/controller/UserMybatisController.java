package com.huihe.sensitive.controller;

import com.huihe.sensitive.pojo.where.WhereUser;
import com.huihe.sensitive.serivice.impl.UserMybatisServiceImpl;
import com.huihe.sensitive.utils.PageResult;
import com.huihe.sensitive.pojo.User;
import io.swagger.annotations.Api;
import io.swagger.annotations.ApiOperation;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.web.bind.annotation.*;

import java.util.List;


/**
 * 用户
 *
 * @author Jacques·Fry
 * @version 1.0.0
 * @since 2021/2/1 9:20
 */
@RestController
@Api(tags = "用户服务-MybatisPlus")
@RequestMapping("/user/mybatis")
public class UserMybatisController {

    @Autowired
    private UserMybatisServiceImpl userService;

    @ApiOperation("保存一个用户对象")
    @PostMapping("/save")
    public User save(@RequestBody User user) {
        return userService.save(user);
    }

    @ApiOperation("根据ID获取一个对象")
    @GetMapping("/get/{id}")
    public User get(@PathVariable(name = "id") int id) {
        return userService.get(id);
    }

    @ApiOperation("获取全部对象")
    @GetMapping("/findAll")
    public List<User> findAll() {
        return userService.findAll();
    }

    @ApiOperation("分页查询")
    @GetMapping("/queryUser")
    public PageResult<User> queryUser(WhereUser where) {
        return userService.queryUser(where);
    }
}
